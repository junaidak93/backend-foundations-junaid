namespace  NotesApi.Helpers;

public static class Constants 
{
    public const string KEY_PAGESIZE = "Paging:PageSize";
    public const string COOKIE_REFRESHTOKEN = "REFRESH_TOKEN";
    public const string KEY_SECRET = "Enc:Secret";
    public const string KEY_SALT = "Enc:Salt";
    public const string BEARER = "Bearer";
    public const string CLAIM_USERID = "userId";
    public const string KEY_CONNECTIONSTRING = "DefaultConnection";
    public const string KEY_JWTKEY = "Jwt:Key";
    public const string KEY_JWTISSUER = "Jwt:Issuer";
    public const string KEY_JWTAUDIENCE = "Jwt:Audience";
    public const string KEY_GRACEPERIODINMINUTES = "Jwt:GracePeriodInMinutes";
}